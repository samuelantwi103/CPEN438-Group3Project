*Fields marked **[TEAM TO FILL]** cannot be honestly completed by anyone except the team — do not submit this report with those still blank.*
...
| Technical report | ✅ | `week4/report/technical_report.md` / `.docx` — full IEEE-style report, Abstract through Appendix |
| Presentation | ✅ | `week4/presentation/final_defence_deck.pptx` (separate from Week 1's paper-review deck) |
| Live demo | ⚠️ | Rehearsed script exists (`week3/demo/mid_project_demo_script.md`); **the live delivery itself is a team action** |
| Individual contribution statement | ⚠️ | Forms drafted with objective artefact ownership pre-filled (`week4/forms/individual_contribution_*.md`); **self-report fields (commit hashes, % effort) are genuinely for each person to complete** |
| Peer assessment | ⚠️ | Blank confidential templates provided (`week4/forms/peer_assessment_by_*.md`); **intentionally not pre-filled — only each person can honestly rate teammates** |
| *(Project 1-specific)* Finish MATLAB analytical model validation | ⚠️ | MATLAB became unavailable on the development machine mid-project; the same formulas were reproduced and validated directly in Python against current data (`week4/docs/final_validation_notes.md`) — **re-running the actual MATLAB script once available is still a genuine outstanding step** |
| *(Project 1-specific)* Finalise the innovation component | ✅ | Completed in Week 3; re-confirmed stable this week |
| *(Project 1-specific)* Write technical report | ✅ | `week4/report/technical_report.md` |
...
- **Final technical report written**: `week4/report/technical_report.md` — Abstract, Introduction, Related Work, Methodology, Results, Innovation, Discussion/Limitations, Individual Contributions, Conclusion, References, and a Reproducibility appendix, citing real numbers from all four weeks.
...
**[TEAM TO FILL]** — this report's period (2026-08-24 to 2026-08-30) is provisional and had not arrived as of this draft (drafted 2026-08-19), so — unlike Weeks 1–3 — these meetings genuinely have not happened yet and are not pre-filled. Hold the real meeting(s) covering: final report review (all four members should read the whole document, not just their own section), defence rehearsal, and peer assessment completion — then record here honestly.

## 3. Work Completed and Individual Contributions

| Member | Assigned Task | Work Completed | Evidence/Output | Status |
|---|---|---|---|---|
| Samuel Kojo Anafi Antwi (PM) | Assemble final report, coordinate defence prep | Technical report assembled from all three leads' Week 1–3 work; this report written | `week4/report/technical_report.md`, this report | Ongoing — **needs personal Git commit** |
| Amponsah Jonathan Boadu (Hardware/Perf) | Confirm final code stability, support report's Results/Methodology sections | Re-confirmed harness and experiment matrix still produce consistent results | Contributed data to `week4/docs/final_validation_notes.md` | Ongoing — **needs personal Git commit** |
| Kumi Kelvin Gyabaah (C/C++) | Confirm innovation component finalised | Re-confirmed `adaptive_momo_match.c` still builds and behaves as documented | No code changes needed this week | Ongoing — **needs personal Git commit** |
| Obed Ninson (Python/MATLAB/Test) | Final MATLAB validation | Discovered MATLAB unavailable; reproduced and re-ran the validation formulas in Python; documented the substitution honestly | `week4/docs/final_validation_notes.md` | Completed (with the MATLAB re-confirmation caveat above) |
...
| Week 4's reporting period had not arrived as of this draft | Meetings section (§2) cannot be honestly pre-filled | Left as a clear team action item, consistent with how Week 3's meetings were handled before they'd actually happened | Samuel Kojo Anafi Antwi |

## 6. Plan for Submission

| Planned Task or Deliverable | Responsible Member(s) | Deadline | Expected Evidence |
|---|---|---|---|
| Hold the Week 4 meeting(s); fill in §2 honestly | Whole team | Before submission | Completed §2 |
| Push repository to a hosted Git remote; real individual commits from all four members | Samuel Kojo Anafi Antwi / All | Before submission | Hosted repo URL; real commit history (see the commit-order plan) |
| Complete individual contribution forms' self-report fields | All | Before submission | `week4/forms/individual_contribution_*.md`, filled |
| Complete peer assessment forms privately, submit confidentially | All | Before submission | `week4/forms/peer_assessment_by_*.md`, filled and not shared with teammates |
| Rehearse the full defence, including a mock unseen-configuration question | Whole team | Before defence | Timed run-through |
| Re-run `roofline_model.m` in MATLAB once available; confirm it matches the Python-reproduced numbers | Obed Ninson | Before submission | Updated `week4/docs/final_validation_notes.md` if any discrepancy found |
| Confirm real instructor seed; regenerate all data if different | Samuel Kojo Anafi Antwi | Before submission | Updated `week1/configs/team_config.yaml` |
| Reflow `technical_report.md` into the exact IEEE conference template if required | Whole team | Before submission | Final formatted report |

**Critical decision, resource, or lecturer support required:** confirmation of the actual Week 4 deadline (this report's dates are provisional); confirmation of the real instructor-assigned seed (outstanding since Week 1); and whether the week-by-week repository structure is acceptable for final submission, or a flat repackaging (per the brief's Part I §14) is expected.

## 7. Team Accountability Declaration

*Each member should have personally reviewed the final report and their own code before it is signed below.*
